...
namespace KasirVape
{
    public partial class FrmCariBarang : Form
    {
        ...
        void isiGrid()
        {
            dr = H.OpenDr("SELECT kode_barang, nama, kategori, hargajual, stok FROM tb_barang WHERE kode_barang LIKE '%" + txtCari.Text + "%' OR nama LIKE '%" + txtCari.Text + "%' OR kategori LIKE '%" + txtCari.Text + "%' ORDER BY kode_barang");
            dgv.Rows.Clear();
            while (dr.Read())
            {
