...
namespace KasirVape
{
    public partial class FrmKasTambah : Form
    {
        ...
        private void btnSimpan_Click(object sender, EventArgs e)
        {
            ...
            if (editState)
            {
                MySqlCommand cmd = new MySqlCommand("UPDATE tb_kas SET faktur=@faktur, tanggal=@tanggal, jenis=@jenis, kategori=@kategori, pemasukan=@pemasukan, pengeluaran=@pengeluaran, keterangan=@keterangan, username=@username WHERE faktur=@faktur", H.getConnection());
            ...
            {
                MySqlCommand cmd = new MySqlCommand("INSERT INTO tb_kas (faktur, tanggal, jenis, kategori, pemasukan, pengeluaran, keterangan, username) VALUES(@faktur, @tanggal, @jenis, @kategori, @pemasukan, @pengeluaran, @keterangan, @username)", H.getConnection());
                cmd.Parameters.AddWithValue("faktur", txtFaktur.Text);
                cmd.Parameters.AddWithValue("tanggal", txtTanggal.Value);
                cmd.Parameters.AddWithValue("jenis", txtJenis.Text);
                cmd.Parameters.AddWithValue("kategori", txtKategori.Text);
